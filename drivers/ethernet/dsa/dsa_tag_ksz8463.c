 ...
 *   01b - Packet received on user port 1.
 *   10b - Packet received on user port 2.
 *   11b - Packet received on both user port 1 and user port 2. These packets
 *         are dropped as the DSA core currently does not support redirecting
 *         packets to multiple interfaces.
 *
 * Additionally, the ingress tag includes 802.1p priorities 0-3 encoded in
 * zero-indexed bits 3:2. These are all left set to 0.
 *
...
#define KSZ8463_NUM_USER_PORTS 2
...
static struct net_if *dsa_tag_ksz8463_recv(struct net_if *iface, struct net_pkt *pkt)
{
	 ...
	 * not part of the frame here. As such, the tag is the very last
	 * byte in the packet
	 */
