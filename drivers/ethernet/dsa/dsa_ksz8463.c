...
struct ksz8463_config {
	...
	/* SPI lock timeout */
	const uint16_t spi_lock_timeout;
...
static inline bool ksz8463_is_cpu_port(const struct device *ptdev)
{
	struct net_if *iface = net_if_lookup_by_dev(ptdev);
...
static int ksz8463_spi_lock(const struct spi_dt_spec *spi)
{
	...
	return k_mutex_lock(&data->spi_mutex, K_MSEC(cfg->spi_lock_timeout));
}
...
static int ksz8463_spi_read_raw(const struct spi_dt_spec *spi, uint16_t addr, void *dst, size_t n)
{
	...
	static_assert(!KSZ8463_SPI_CMD_RD, "");
...
/* Read 16 bites from register, aligning addr if required. The SPI mutex should
 * be held
 */
static int ksz8463_spi_readw_raw(const struct spi_dt_spec *spi, uint16_t addr)
{
	...
	static_assert(sizeof(int) > sizeof(be16), "Potential overflow");
	return (int)be16;
}
...
static int ksz8463_spi_readb_raw(const struct spi_dt_spec *spi, uint16_t addr)
{
	...
	/* Device supports accesses noly at 4 byte boundaries */
	if (addr & 0x03u) {
				   ...
				   uint32_t mask)
{
	...
	/* All 1-bits in bits must be set in mask */
...
static int ksz8463_eee_en_disen(const struct device *ptdev, bool enable, bool lock_spi)
{
	...
	static_assert(KSZ8463_PCSEEEC_P1_NEXT_PG_EN == BIT(0), "");
	static_assert(KSZ8463_PCSEEEC_P2_NEXT_PG_EN == BIT(1), "");
...
static inline int ksz8463_restart_autoneg(const struct device *ptdev)
{
	const struct device *dev;
	const struct ksz8463_config *cfg;
	const struct dsa_port_config *dsa_cfg = ptdev->config;
...
static void ksz8463_chip_isr_work(struct k_work *work)
{
	...
	/* Chip's internal interrupt state may change even though the driver's
	 * ISR won't be invoked here due to the irq_lock. Repeatedly act on the
	 * interrupt state until the LCIS interrupt is low
	 */
	do {
...
static void ksz8463_autoneg_work(struct k_work *work)
{
	...
	if (ret & KSZ8463_PxMBSR_AUTONEG_CPLT) {
		...
		LOG_WRN("Auto-negotiation timed out");
		ret = ksz8463_configure_fix_link(ptdev);
	} else {
...
static int ksz8463_validate_phy_mode(const struct device *ptdev)
{
	...
	if (ret) {
		LOG_ERR("Invalid PHY mode %s for CPU port", dsa_cfg->phy_mode);
	}
	return ret;
}
...
static int ksz8463_port_init(const struct device *ptdev)
{
	...
	if (IS_ENABLED(CONFIG_MULTITHREADING)) {
		/* Ensure the write is observed even should ksz8463_switch_setup
...
static int ksz8463_switch_setup(const struct dsa_switch_context *dsa_switch_ctx)
{
	...
	if (!ret) {
		if (IS_ENABLED(CONFIG_MULTITHREADING)) {
			/* Synchronize with releases in ksz8463_port_init */
...
#define KSZ8463_PORT_INIT(pt, n)                                                                   \
	static_assert(DT_PROP(pt, microchip_autoneg_poll_interval) <                               \
	...
	static_assert(!DT_NODE_HAS_PROP(pt, ethernet) ||                                           \
	...
	static_assert(DT_NODE_HAS_PROP(pt, ethernet) || DT_NODE_HAS_PROP(pt, phy_handle),          \
...
#define KSZ8463_INIT(n)                                                                            \
	...
	static_assert(DT_INST_CHILD_NUM(n) < UINT8_MAX, "");                                       \
                                                                                                   \
	/* Overflow? */                                                                            \
	static_assert(DT_INST_PROP(n, microchip_spi_lock_timeout) <= UINT16_MAX,                   \
		      "microchip,spi-lock-timeout may be at most 65535");                          \
                                                                                                   \
	/* Overflow? */                                                                            \
	static_assert(DT_INST_PROP(n, microchip_spi_operational_timeout) <= UINT16_MAX,            \
		      "microchip,spi-operational-timeout may be at most 65535");                   \
                                                                                                   \
	/* Must be exactly one CPU port */                                                         \
	static_assert(DT_INST_FOREACH_CHILD_STATUS_OKAY_SEP(n, KSZ8463_IS_CPU_PORT, (+)) == 1u,    \
		      "The ethernet phandle must be set for exactly one "                          \
		      "port - the CPU port.");                                                     \
                                                                                                   \
	/* Remaining ports - the user ports - all need a phy-handle */                             \
	static_assert(DT_INST_FOREACH_CHILD_STATUS_OKAY_SEP(n, KSZ8463_IS_USER_PORT, (+)) ==       \
			      DT_INST_CHILD_NUM_STATUS_OKAY(n) - 1u,                               \
		      "At least one user port is missing the phy-handle "                          \
		      "property");                                                                 \
                                                                                                   \
	static_assert(DT_INST_ENUM_IDX(n, microchip_port_led_mode) <= BIT_MASK(2),                 \
		      "Invalid choice of LED mode");                                               \
                                                                                                   \
	static_assert(DT_INST_ENUM_IDX(n, microchip_fixed_link_speed) <= KSZ8463_SPEED_MAX,        \
	...
	static const struct ksz8463_config ksz8463_config_##n = {                                  \
		...
		.spi_lock_timeout = DT_INST_PROP(n, microchip_spi_lock_timeout),                   \
	...
	struct ksz8463_prv_data ksz8463_prv_data_##n = {                                           \
		.dev = DEVICE_DT_INST_GET(n),                                                      \
                                                                                                   ...
                                                                                                   \
	static_assert(DT_INST_CHILD_NUM_STATUS_OKAY(n), "No ports enabled");                       \
                                                                                                   \
